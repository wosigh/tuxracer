...
void init_high_scores( void ) 
{
    ...
    if ( get_high_score_file_name( score_file, sizeof(score_file) ) == 0 ) {
        ...
        if (score_stream != NULL) {
            ...
            fread( &wantedhash, sizeof(uint32_t), 1, score_stream);
            while (fread( &this_score, sizeof(this_score), 1, score_stream)) {
                h = hash((uint8_t *)&this_score, sizeof(this_score), h);
            ...
            if(wantedhash != h)
            {
                score_table = create_hash_table(); // We leak, but who cares?
            }
        }
    }
}
...
void write_high_scores( void )
{
    ...
    if ( get_high_score_file_name( score_file, sizeof(score_file) ) == 0 ) {
        ...
        fwrite( &h, sizeof(hash), 1, score_stream );
...
void init_saved_games( void ) 
{
         ...
         cur_dir_file = get_next_list_elem( dir_file_list, cur_dir_file ) )
    {
        ...
        if (get_sav_index(cur_dir_filename, &sav_index)) {
            ...
            if (save_stream != NULL) {
                uint32_t wantedh, h = 0;
                if ( fread( &wantedh, sizeof(h), 1, save_stream ) != 1 ) 
                {
                    print_warning( IMPORTANT_WARNING,
                                  "`%s' is not a valid saved game file",
                                  file_name );
                ...
                while (fread( &this_save, sizeof(this_save), 1, save_stream)) {
                    h = hash((uint8_t *)&this_save, sizeof(this_save), h);
                ...
                {
                    for (i=0; i<DIFFICULTY_NUM_LEVELS; i++)
                        results_save_table[i] = create_hash_table(); // Leak
                }
...
void reinit_saved_game( char * player )
{
	int i;
    for (i=0; i<DIFFICULTY_NUM_LEVELS; i++) {
        results_save_table[i] = create_hash_table();
...
void delete_high_score( void )
{
	int i;
    for (i=0; i<DIFFICULTY_NUM_LEVELS; i++)
        results_save_table[i] = create_hash_table(); // Leak, but who seriously cares?
    score_table = create_hash_table(); // We leak, but who seriously cares?
    truncate_and_init_save_file( "noname" );
	truncate_and_init_save_file( "nonameSpeedOnly" );// Force score suppression
}
                                ...
                                (hash_entry_t*)&event_table ) ) 
        {
                                         ...
                                         sizeof(save_file) ) == 0 ) 
            {
                ...
                if ( save_stream == NULL ) {
                                            ...
                                            (hash_entry_t*)&cup_table ) ) 
                    {
                                                ...
                                                (hash_entry_t*)&race_table ) )
                        {
                            ...
                            while ( next_hash_entry( race_scan_ptr, NULL,
                                                    (hash_entry_t*)&this_save ) )
                            {
                                h = hash( (uint8_t *)this_save, sizeof(save_info_t), h );
                            }
                    ...
                    fwrite( &h, sizeof(h), 1, save_stream );
