...
static bool_t facebookPublishDialogDisplayed;
...
void displayFacebookDialog() {
	if (!facebookPublishDialogDisplayed && g_game.rankings_loaded) {
		fb_click_cb(fb_btn,NULL);
		facebookPublishDialogDisplayed=True;
	}
}
...
void draw_status_msg( int x_org, int y_org, int box_width, int box_height )
{
    ...
    if ( g_game.practicing ) {
        ...
        if ( !get_saved_race_results( plyr->name,
                                     ...
                                     &score ) )
        {
            /* Don't display any scores and hides facebook button if no score saved */
			button_set_visible(fb_btn, false);
            return;
        } else { 
			const char* mode = g_game.is_speed_only_mode?"Speed Only":"Classic";
			if (isRaceAllowedToPublishOnFacebook(mode,data->name)) button_set_visible(fb_btn, true);
			else button_set_visible(fb_btn, false);
		}
...
static void set_widget_positions_and_draw_decorations()
{
	...
	button_set_position(fb_btn,make_point2d(MARGIN_WIDTH,250 ) );
...
static void race_select_init(void)
{
	...
	fb_btn = button_create( dummy_pos,
						   134, 31, 
						   NULL, 
						   "");
	button_set_image(fb_btn, "postFB", make_point2d(0,0), make_point2d(134.0/256.0, 1 ), white);
	button_set_clicked_image(fb_btn, "postFB_down", make_point2d(0, 0 ), make_point2d(134.0/256.0, 1), white);
	button_set_click_event_cb( fb_btn, fb_click_cb, NULL );
	if (g_game.practicing) button_set_visible( fb_btn, True ); else button_set_visible( fb_btn, False );
