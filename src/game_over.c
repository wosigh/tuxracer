...
static char* challengeRanking = NULL;
static scalar_t friendsPercent,countryPercent,worldPercent,challengePercent;
...
void draw_game_over_text( void )
{
    ...
    //display rankings if needed
        else if ( !g_game.race_aborted && g_game.practicing && g_game.needs_save_or_display_rankings) 
        {
                ...
                !get_font_binding( "race_stats", &font ) )
            {
				...
				/* Challenge */
				//On affiche les résultats au challenge que si le mec a battu son meilleur score
				if (did_player_beat_best_results()) {
					glPushMatrix();
					{
						...
						bind_font_texture( stat_label_font );
						draw_string( stat_label_font, Localize("Total: ","") );
						
						sprintf( buff, "%s (%.1f%%)", strcmp(challengeRanking,"-1")?(const char*) challengeRanking:Localize("Not Ranked","") ,strcmp(challengeRanking,"-1")?challengePercent:0.0 );
               ...
               did_player_beat_best_results() ) 
    {
        if ( !g_game.race_aborted && g_game.practicing && g_game.needs_save_or_display_rankings) {
            string = "";
        } else {//A best score has just been established
			const char* mode = g_game.is_speed_only_mode?"Speed Only":"Classic";
			allowRaceToPublishOnFacebook(mode, g_game.race.name);
			string = Localize("You beat your best score!","");
		}
...
void displaySavedAndRankings(const char* msg, const char* friends, const char* country, const char* world, const char* challenge, scalar_t friendsPercentage, scalar_t countryPercentage , scalar_t worldPercentage, scalar_t challengePercentage) {
    free(friendsRanking);
    friendsRanking = strdup(friends);
    free(countryRanking);
    countryRanking = strdup(country);
	g_game.countryRanking = countryRanking;
    free(worldRanking);
    worldRanking = strdup(world);
	g_game.worldRanking = worldRanking;
	free(challengeRanking);
    challengeRanking = strdup(challenge);
	g_game.totalRanking = challengeRanking;
    friendsPercent = friendsPercentage;
    countryPercent = countryPercentage;
    worldPercent = worldPercentage;
	challengePercent = challengePercentage;
	g_game.rankings_loaded=true;
}
